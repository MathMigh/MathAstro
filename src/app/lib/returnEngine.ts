...
import { generateTraditionalReturnReport } from "./predictiveReports";
...
const RETURN_CONFIG: Record<ReturnChartType, ReturnCalculationConfig> = {
  ...
  lunar: {
    bodyId: 1,
    searchWindowHours: 384,
    coarseStepMinutes: 120,
  },
};

const RETURN_EXACTNESS_TOLERANCE_DEGREES = 0.0001;
const MAX_TRUE_BRACKET_DELTA_JUMP_DEGREES = 90;
...
function getTimeParts(time: string | number | undefined): {
  ...
  const totalSeconds = Math.round((safeDecimalTime - hours) * 3600);
  const minutes = Math.floor(totalSeconds / 60);
  const seconds = totalSeconds % 60;

  if (minutes === 60) {
    return { hours: hours + 1, minutes: 0, seconds };
  }
...
function getMomentForBirthDate(birthDate: BirthDate): {
  ...
  const { hours, minutes, seconds } = getTimeParts(birthDate.time);
...
async function getReturnLongitudeDelta(
  ...
  const cacheKey = localMoment.valueOf().toString();
...
async function refineBracket(
  ...
  for (let iteration = 0; iteration < 32; iteration += 1) {
    ...
    if (Math.abs(highMoment.diff(lowMoment, "seconds")) <= 1) {
      break;
    }
...
async function findReturnMoment(
  ...
  for (
    ...
    if (Math.abs(delta) < RETURN_EXACTNESS_TOLERANCE_DEGREES) {
      return {
    ...
    if (
      ...
      Math.abs(sample.delta - previousSample.delta) <=
        MAX_TRUE_BRACKET_DELTA_JUMP_DEGREES
    ) {
  ...
  if (Math.abs(closestSample.delta) < RETURN_EXACTNESS_TOLERANCE_DEGREES) {
    return {
      returnMoment: closestSample.moment,
      timezone,
    };
  }

  throw new Error(
    "Nao foi possivel localizar um retorno real na janela pesquisada."
  );
}
...
export async function calculateReturnChartData(
  ...
  const [natalChart, returnChart] = await Promise.all([
    calculateBirthChart(birthDate),
    calculateBirthChart(returnBirthDate),
  ]);
  const traditionalReport = generateTraditionalReturnReport({
    natalChart,
    returnChart,
    returnType,
    targetDate,
