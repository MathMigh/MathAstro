import type { HoraryAIHandoff, HoraryDossier, HoraryTestimony } from "./types";

export const HORARY_AI_PROMPT_VERSION = "MATHASTRO-HORARY-AI-PTBR-v3.0";
export const HORARY_AI_CONTRACT_VERSION = "horary-ai-handoff/2.0";

/**
 * Prompt absoluto da camada interpretativa. O motor fecha astronomia, casas e
 * mecanismos sempre que possível; a IA resolve somente a parte semântica e
 * contextual que, por natureza, não pode ser reduzida a uma tabela finita.
 */
export const HORARY_ABSOLUTE_AI_SYSTEM_PROMPT = `
...
function allSourceIds(d:HoraryDossier):string[]{
  const fromTests=(xs:HoraryTestimony[])=>xs.flatMap(x=>x.sourceIds);
  return unique([
    ...d.topicAnalysis.houses.flatMap(x=>x.sourceIds),
    ...d.topicAnalysis.semanticFrame.compiledRoles.flatMap(x=>x.sourceIds),
    ...d.significators.flatMap(x=>x.sourceIds),
    ...d.mediation.flatMap(x=>x.sourceIds),
    ...fromTests(d.testimonies),
    ...fromTests(d.considerations),
  ]).sort();
}

function ambiguityQuestions(d:HoraryDossier):string[]{
  const out:string[]=[];
  for(const a of d.question.semanticAmbiguities??[]){
    if(a.blocking===false) continue;
    const choices=a.candidates.map(c=>`${c.meaning}${c.anchorRole?` de ${c.anchorRole}`:""}`).join(" / ");
    out.push(`Na expressão “${a.phrase}”, qual sentido concreto é o correto: ${choices}?`);
  }
  for(const x of d.topicAnalysis.unresolvedContext) out.push(`Forneça/defina o contexto necessário: ${x}.`);
  return unique(out);
}

export function buildHoraryAIHandoff(d:HoraryDossier):HoraryAIHandoff {
  const unresolved=unique([
    ...d.topicAnalysis.unresolvedContext,
    ...d.topicAnalysis.semanticFrame.unresolved,
    ...(d.question.semanticAmbiguities??[]).filter(a=>a.blocking!==false).map(a=>`ambiguidade: ${a.phrase}`),
  ]);
  const tasks:HoraryAIHandoff["interpretiveTasks"]=[];

  if(unresolved.length){
    tasks.push({
      id:"resolve-semantics",
      kind:"semantic_resolution",
      question:"Resolva somente as ambiguidades de linguagem/contexto necessárias para determinar papéis, relações e casas; se não houver base, peça clarificação.",
      blocking:true,
      allowedActions:["usar contexto fornecido","escolher entre candidatos ontologicamente legítimos","pedir clarificação mínima"],
      forbiddenActions:["adivinhar uma casa pela palavra-chave","inventar relação derivada","alterar casas já compiladas"],
      sourceIds:["M-HOUSE-TURNING","F-HT","MA-OPS"],
    });
  }

  tasks.push({
    id:"synthesize-judgement",
    kind:"contextual_judgement",
    question:"Sintetize o julgamento causal por subpergunta a partir dos fatos calculados, sem score totalizador.",
    blocking:false,
    allowedActions:["priorizar testemunhos causais","interpretar recepções e dignidades conforme o contexto","separar evento de estado"],
    forbiddenActions:["recepção=evento","dignidade=evento","contagem simples de testemunhos"],
    sourceIds:["M-HORARY-CURRENT","F-HT","MA-OPS"],
  });

  if(d.chronology?.length || d.decisionPlan.intents.includes("timing")){
    tasks.push({
      id:"select-timing",
      kind:"timing_selection",
      question:"Se o evento estiver estabelecido, selecione a unidade temporal plausível e mantenha separado o arco simbólico da cronologia astronômica.",
      blocking:false,
      allowedActions:["usar calibração passada","usar contexto real","comparar variantes documentadas"],
      forbiddenActions:["dar timing antes de provar evento","converter dias efeméricos automaticamente em unidades simbólicas"],
      sourceIds:["M-HORARY-CURRENT","F-HT","G-SUPP"],
    });
  }

  if(d.coverage.status==="manual_context"){
    tasks.push({
      id:"manual-context-topic",
      kind:"symbolic_synthesis",
      question:`O tópico ${d.question.topic} exige síntese contextual; use o atlas, o plano decisório e apenas regras documentadas.`,
      blocking:false,
      allowedActions:["compor relações semânticas","explicar limites","devolver SOURCE_RULE_REQUIRED"],
      forbiddenActions:["criar regra tópica inexistente","forçar resposta binária"],
      sourceIds:["M-HORARY-CURRENT","F-HT","G-SUPP","MA-OPS"],
    });
  }
  ...
  tasks.push({
    id:"report",
    kind:"reporting",
    question:"Produza relatório claro que permita auditar por que a conclusão segue do mapa e do método.",
    blocking:false,
    allowedActions:["explicitar variantes","explicitar incerteza","separar subperguntas"],
    forbiddenActions:["ocultar conflito de fonte","atribuir certeza maior que a evidência","recalcular números"],
    sourceIds:["MA-OPS"],
  });
  ...
  return {
    ...
    deterministicBoundary:{
      closedByEngine:[
        "astronomia e velocidades","cúspides Regiomontanus e regentes tradicionais","turning já compilado","dignidades essenciais/acidentais","recepções direcionais","aspectos e perfeições","antíscios calculados","Lua/VOC","cronologia Swiss Ephemeris","estações/ingressos/combustão","testemunhos mecânicos já materializados"
      ],
      aiMayInterpret:[
        "sentido humano da pergunta","papéis relacionais ainda ambíguos","qual regra documentada é pertinente ao contexto","peso causal/contextual dos testemunhos","síntese de subperguntas","unidade simbólica de timing","redação final"
      ],
      aiMustNotAlter:[
        "graus, velocidades e casas calculadas","regentes tradicionais","ordem cronológica calculada","status aplicativo/separativo materializado","fatos do contexto fornecidos pelo usuário","sourceIds e conflitos de fonte"
      ],
    },
    sourceHierarchy:[
      {priority:1,source:"Marcos Monteiro",policy:"fonte principal prática/atual e casos diretos; preservar nuances contextuais"},
      {priority:2,source:"John Frawley",policy:"fonte principal publicada para método e casos; canônica quando compatível com Marcos"},
      {priority:3,source:"Luiz Gonzaga de Carvalho Neto",policy:"suplemento documentado; divergências ficam como variantes explícitas"},
      {priority:4,source:"MathAstro",policy:"contrato operacional, isolamento, segurança e não-invenção"},
    ],
    requiredSourceIds:allSourceIds(d),
    interpretiveTasks:tasks,
    unresolved,
    clarificationQuestions:ambiguityQuestions(d),
    machineJudgementIsAdvisory:true,
    outputContract:{
      status:["JUDGED","NEEDS_CLARIFICATION","DESCRIPTIVE_ONLY","SOURCE_RULE_REQUIRED"],
      answer:["YES","NO","MIXED","UNKNOWN","DESCRIPTIVE_ONLY"],
      requiredFields:["status","questionReframed","semanticResolution","answer","causalChain","sourceVariants","usedSourceIds","unresolvedSubjectivity","confidence","clarificationNeeded","sourceRuleRequired","reportText"],
      confidence:["high","medium","low"],
    },
  };
}

export function renderHoraryAIUserPrompt(d:HoraryDossier):string {
  const handoff=buildHoraryAIHandoff(d);
  const payload={
    question:d.question,
    coverage:d.coverage,
    ontology:d.ontology,
    decisionPlan:d.decisionPlan,
    semanticFrame:d.topicAnalysis.semanticFrame,
    houseAssignments:d.topicAnalysis.houses,
    significators:d.significators,
    receptions:d.receptions,
    directPerfections:d.directPerfections,
    antiscialContacts:d.antiscialContacts,
    mediation:d.mediation,
    chronology:d.chronology??[],
    lunarSequence:d.lunarSequence,
    moonVoidOfCourse:d.moonVoidOfCourse,
    testimonies:d.testimonies,
    considerations:d.considerations,
    auxiliary:d.auxiliary,
    machineJudgement:d.judgement,
    provenance:d.provenance,
    aiHandoff:handoff,
  };
  return `Analise o dossiê abaixo obedecendo integralmente ao system prompt ${HORARY_AI_PROMPT_VERSION}. Não recalcule o mapa. Se houver ambiguidade bloqueadora, faça somente as perguntas de clarificação indispensáveis.\n\nDOSSIÊ_JSON:\n${JSON.stringify(payload,null,2)}`;
}
